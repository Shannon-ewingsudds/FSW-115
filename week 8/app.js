...
async function getData() {
    const listRes = await axios.get("http://api.bryanuniversity.edu/shannonewingsudds/list");
    const catRes = await axios.get("https://catfact.ninja/fact");
    const pendingDataPromises = [listRes, catRes.data.fact];
...
function displayFacts(data) {
    let catFacts = document.createElement('h3');
    catFacts.textContent = `Cat Facts: ${data}`;
    document.body.appendChild(catFacts);
}
...
async function deleteLi() {
    ...
    let response = await axios.delete(`http://api.bryanuniversity.edu/shannoneewingusdds/list/${listID}`);
    location.reload();
}
...
function updateLi() {
    ...
    axios.put(`http://api.bryanuniversity.edu/shannonewingsudds/list/${item.id}`, updates)
